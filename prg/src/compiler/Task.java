...
public class Task {
	...
	/** Number of registers if regalloc phase is performed. */
	...
	public Task(String[] args) {
		...
		for (int argc = 0; argc < args.length; argc++) {
			if (args[argc].startsWith("-")) {
				...
				if (args[argc].startsWith("--numregs=")) {
					if("regalloc".equals(phase)) {
						try {
							numRegs = Integer.parseInt(args[argc].replaceFirst("--numregs=", ""));
						} catch (Exception e) {
							Report.warning("Specify the register number. Default is 4 registers.");
						}
					}
					continue;
				}
